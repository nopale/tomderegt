<div class="flex-ns w-100 relative flex-wrap dn " style="margin-top: -1px;">
<div class="pseudo-border absolute absolute-top"></div>
        <div class=" alt-link-wrapper flex flex-column justify-start items-center relative">
			<div class="h-100 w-100 flex justify-center items-center relative">
				<a href="/clients" class="inline-flex link  h-100 justify-center tom-nav-link">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 184.3 169.8">
						<g id="Layer_2" data-name="Layer 2">
							<g id="Layer_1-2" data-name="Layer 1">
							<g>
								<path class="cls-1" d="M113.4,18.1l40.1,64.1,5.4-3.4L118.8,14.7a31.5,31.5,0,0,0-53.3,0L57.8,27.1,56,19.6,49.9,21,55,43.5,70.9,18.1a25,25,0,0,1,42.5,0Z"/>
								<path class="cls-1" d="M179.4,111.6l-6.6-10.5,7.6,1.8,1.4-6.2-22.5-5.3,14.8,23.5a25.1,25.1,0,0,1,.7,25.5,24.8,24.8,0,0,1-22,12.9H75.2v6.3h77.6a31.3,31.3,0,0,0,26.6-48Z"/>
								<path class="cls-1" d="M9.5,140.4a24.8,24.8,0,0,1,.7-25.5l39-62.1-5.4-3.4L4.9,111.6A30.8,30.8,0,0,0,4,143.4a30.9,30.9,0,0,0,27.5,16.2H49.2l-5.5,5.8,4.5,4.4,15.9-16.5H31.5A24.8,24.8,0,0,1,9.5,140.4Z"/>
								<path class="cls-1" d="M94,74.5c5.9,0,9.5,3.7,10.7,10.6l6.4-1.1c-1.1-9.3-8-15.5-17.1-15.5-11.3,0-18.8,9.2-18.8,23s7.4,23.1,18.8,23.1a16.6,16.6,0,0,0,11.9-4.4c3.1-3,4.9-7.1,5.3-12.2l-6.5-1c-.8,7.6-4.5,11.5-10.7,11.5s-12.3-6.4-12.3-17S86.3,74.5,94,74.5Z"/>
							</g>
							</g>
						</g>
					</svg>

					<p class="absolute shadow-text ma0 f4 f3-l ttu">clients</p>
				</a>
				
				<div class="pseudo-border absolute absolute-bottom"></div>
			</div>
		
			<div class="pseudo-border absolute absolute-bottom"></div>
            <!-- <div class="pseudo-border vertical absolute absolute-top absolute-right"></div> -->
		</div>

		<div class=" alt-link-wrapper flex flex-column justify-start items-center relative">
			<div class="h-100 w-100 flex justify-center items-center relative">
				<a href="/category/typography" class="inline-flex link  h-100 justify-center tom-nav-link">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 152.4 152.4">
                        <g id="Layer_2" data-name="Layer 2">
                            <g id="Layer_1-2" data-name="Layer 1">
                            <g>
                                <path class="cls-1" d="M143.1,129.5V21.1A10.7,10.7,0,1,0,130.4,8.4H21.1A10.7,10.7,0,1,0,8.4,21.1V130.4a10.7,10.7,0,1,0,12.7,12.7H129.5a11.5,11.5,0,0,0,11.3,9.3,11.6,11.6,0,0,0,11.6-11.6A11.5,11.5,0,0,0,143.1,129.5ZM140.8,4.4a6.3,6.3,0,1,1-6.2,6.3A6.3,6.3,0,0,1,140.8,4.4ZM4.4,10.7a6.3,6.3,0,1,1,6.3,6.2A6.3,6.3,0,0,1,4.4,10.7Zm6.3,136.4a6.3,6.3,0,1,1,6.2-6.3A6.3,6.3,0,0,1,10.7,147.1Zm2.2-16.7V21.1a10.8,10.8,0,0,0,8.2-8.2H130.4a10.8,10.8,0,0,0,8.2,8.2V129.5a11.5,11.5,0,0,0-9.1,9.1H21.1A10.8,10.8,0,0,0,12.9,130.4Zm127.9,15.7a5.3,5.3,0,0,1,0-10.5,5.3,5.3,0,1,1,0,10.5Z"/>
                                <g>
                                <path class="cls-1" d="M72.8,115.3H66.4V65.5A22.1,22.1,0,0,0,44.3,43.4H28V37.1H44.3A28.5,28.5,0,0,1,72.8,65.5Z"/>
                                <path class="cls-1" d="M85.1,115.3H78.7V65.5a28.5,28.5,0,0,1,28.5-28.4h16.3v6.3H107.2A22.1,22.1,0,0,0,85.1,65.5Z"/>
                                <path class="cls-1" d="M97.4,115.3H91V65.5a17.3,17.3,0,0,1,17.3-17.2h15.2v6.3H108.3A10.9,10.9,0,0,0,97.4,65.5Z"/>
                                <path class="cls-1" d="M60.5,115.3H54.1V65.5A10.9,10.9,0,0,0,43.2,54.6H28V48.3H43.2A17.3,17.3,0,0,1,60.5,65.5Z"/>
                                </g>
                            </g>
                            </g>
                        </g>
                    </svg>

					<p class="absolute shadow-text ma0 f4 f3-l ttu">typography</p>
				</a>
			
			</div>
		
			<div class="pseudo-border absolute absolute-bottom"></div>
            <!-- <div class="pseudo-border vertical absolute absolute-top absolute-right"></div> -->
		</div>
		
		

		<div class=" alt-link-wrapper flex flex-column justify-start items-center relative">
			<div class="h-100 w-100 flex justify-center items-center relative">
				<a href="/category/advertising" class="inline-flex link  h-100 justify-center tom-nav-link" >
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 148.9 137.2">
						<g id="Layer_2" data-name="Layer 2">
							<g id="Layer_1-2" data-name="Layer 1">
							<g>
								<path class="cls-1" d="M101.5,38l-2.3-2.3a9.1,9.1,0,0,0-6.7-2.8h0a9.5,9.5,0,0,0-6.7,2.8l-83,83.1a9.4,9.4,0,0,0,0,13.4L5,134.4a9.4,9.4,0,0,0,6.7,2.8,9.2,9.2,0,0,0,6.7-2.8l83.1-83a9.5,9.5,0,0,0,2.8-6.7A9.3,9.3,0,0,0,101.5,38ZM13.9,130a3.1,3.1,0,0,1-4.4,0l-2.3-2.3a3.1,3.1,0,0,1-.9-2.2,2.9,2.9,0,0,1,.9-2.2L75.6,54.9l6.7,6.7ZM97,46.9,86.8,57.1l-6.7-6.7L90.3,40.2a3.1,3.1,0,0,1,2.2-.9h0a3.2,3.2,0,0,1,2.3.9L97,42.4a3.2,3.2,0,0,1,.9,2.3A3.1,3.1,0,0,1,97,46.9Z"/>
								<polygon class="cls-1" points="135 13.9 131.8 0 128.6 13.9 114.8 17.1 128.6 20.3 131.8 34.2 135 20.3 148.9 17.1 135 13.9"/>
								<polygon class="cls-1" points="49.4 34.2 52.6 20.3 66.5 17.1 52.6 13.9 49.4 0 46.3 13.9 32.4 17.1 46.3 20.3 49.4 34.2"/>
								<polygon class="cls-1" points="117.4 69.5 114.9 80.4 104 82.8 114.9 85.3 117.4 96.2 119.9 85.3 130.8 82.8 119.9 80.4 117.4 69.5"/>
							</g>
							</g>
						</g>
					</svg>
					<p class="absolute shadow-text ma0 f4 f3-l ttu">advertising</p>
				</a>
				
			</div>
		
			<div class="pseudo-border absolute absolute-bottom"></div>
		</div>

		<div class=" alt-link-wrapper flex flex-column justify-start items-center relative">
			<div class="h-100 w-100 flex justify-center items-center relative">
				<a href="/category/covers" class="inline-flex link  h-100 justify-center tom-nav-link" >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 188.7 138.1">
                            <g id="Layer_2" data-name="Layer 2">
                                <g id="Layer_1-2" data-name="Layer 1">
                                <g>
                                    <path class="cls-1" d="M138.1,4.5V0H0V138.1H138.1v-7.5a64.6,64.6,0,0,0,0-126.1Zm-6.3,127.3H6.3V6.3H131.8Zm6.3-84.9a23.4,23.4,0,0,1,0,37.9Zm0,77.2V88.7a26.8,26.8,0,0,0,0-45.7V11.1a58.2,58.2,0,0,1,0,113Z"/>
                                    <rect class="cls-1" x="67.5" y="-3.8" width="3.2" height="145.67" transform="translate(-28.6 69.1) rotate(-45)"/>
                                </g>
                                </g>
                            </g>
                        </svg>
					<p class="absolute shadow-text ma0 f4 f3-l ttu">covers</p>
				</a>
				
			</div>
		
			<div class="pseudo-border absolute absolute-bottom"></div>
            <!-- <div class="pseudo-border vertical absolute absolute-top absolute-right"></div> -->
        </div>
        
        <div class=" alt-link-wrapper flex flex-column justify-start items-center relative">
			<div class="h-100 w-100 flex justify-center items-center relative">
				<a href="/category/clothing" class="inline-flex link  h-100 justify-center tom-nav-link" >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 178.3 113.1">
                        <g id="Layer_2" data-name="Layer 2">
                            <g id="Layer_1-2" data-name="Layer 1">
                            <g>
                                <path class="cls-1" d="M154,17.7A23.8,23.8,0,0,0,131,0H27.4V6.3H131a17.5,17.5,0,0,1,16.9,13l5.7,22.4-92,.3a56,56,0,0,0-54,41.9L0,113.1H178.3ZM13.7,85.5A49.8,49.8,0,0,1,61.6,48.3l93.6-.3,15,58.7H8.2Z"/>
                                <path class="cls-1" d="M92.7,68.6a8.7,8.7,0,1,0,8.7,8.7A8.7,8.7,0,0,0,92.7,68.6Z"/>
                            </g>
                            </g>
                        </g>
                    </svg>
					<p class="absolute shadow-text ma0 f4 f3-l ttu">clothing</p>
				</a>
				
			</div>
		
			<div class="pseudo-border absolute absolute-bottom"></div>
            <!-- <div class="pseudo-border vertical absolute absolute-top absolute-right"></div> -->
        </div>
        
        <div class=" alt-link-wrapper flex flex-column justify-start items-center relative">
			<div class="h-100 w-100 flex justify-center items-center relative">
				<a href="/about" class="inline-flex link  h-100 justify-center tom-nav-link" >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 162.4 162.4">
						<g id="Layer_2" data-name="Layer 2">
							<g id="Layer_1-2" data-name="Layer 1">
							<g>
								<path class="cls-1" d="M81.2,162.4a81.2,81.2,0,1,1,81.2-81.2A81.3,81.3,0,0,1,81.2,162.4Zm0-156.1a74.9,74.9,0,1,0,74.9,74.9A75,75,0,0,0,81.2,6.3Z"/>
								<path class="cls-1" d="M81.2,136.7A55.5,55.5,0,0,1,25.8,81.2a3.2,3.2,0,0,1,6.3,0,49.1,49.1,0,0,0,98.2,0,3.2,3.2,0,0,1,3.2-3.1,3.1,3.1,0,0,1,3.2,3.1A55.6,55.6,0,0,1,81.2,136.7Z"/>
								<g>
								<path class="cls-1" d="M51.3,38.3c-9,0-14.8,7.2-14.8,18.4S42.5,75,51.3,75s14.9-7.4,14.9-18.3S60.4,38.3,51.3,38.3Zm0,30.8c-5.4,0-8.6-4.6-8.6-12.4s3.2-12.5,8.6-12.5S60,48.9,60,56.7,56.7,69.1,51.3,69.1Z"/>
								<path class="cls-1" d="M111.1,38.3c-9.1,0-14.9,7.2-14.9,18.4s6,18.3,14.9,18.3,14.8-7.4,14.8-18.3S120.1,38.3,111.1,38.3Zm0,30.8c-5.4,0-8.7-4.6-8.7-12.4s3.3-12.5,8.7-12.5,8.6,4.7,8.6,12.5S116.5,69.1,111.1,69.1Z"/>
								<path class="cls-1" d="M77.8,99l5.8-3c3.9-1.8,6.6-3.5,8.2-5.3a9.9,9.9,0,0,0,2.5-6.8c0-5.9-5.4-10.3-12.5-10.3-4,0-7.3,1.1-9.7,3.3A12.4,12.4,0,0,0,68,86h0v.6h0a3.5,3.5,0,0,0,.8,1.6,3,3,0,0,0,2.2,1A2.9,2.9,0,0,0,74,87v-.5c.4-4.6,3.1-7.1,7.7-7.1s6.3,1.8,6.3,4.5-1.9,4.5-8.7,7.7-7.7,4.2-9.2,6.4a13.3,13.3,0,0,0-1.7,7.1v4.4H91.9a2.8,2.8,0,0,0,2.4-2.8,2.9,2.9,0,0,0-2.4-2.9H75.3a4.1,4.1,0,0,1,.6-2.7A4.9,4.9,0,0,1,77.8,99Z"/>
								</g>
							</g>
							</g>
						</g>
					</svg>
					<p class="absolute shadow-text ma0 f4 f3-l ttu">about</p>
				</a>
				
			</div>
			<div class="pseudo-border vertical absolute absolute-top absolute-right dn-ns"></div>
			<div class="pseudo-border absolute absolute-bottom"></div>
           
		</div>

		<div class=" alt-link-wrapper flex flex-column justify-start items-center relative">
			<div class="h-100 w-75 flex justify-center items-center relative">
				<a href="/category/identity" class="inline-flex link  h-100 justify-center tom-nav-link identity-link" >
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 113.6 156.9">
					<g id="Layer_2" data-name="Layer 2">
						<g id="Layer_1-2" data-name="Layer 1">
						<g>
							<path class="cls-1" d="M107.1,156.9H6.5A6.5,6.5,0,0,1,0,150.4V65.8A11.9,11.9,0,0,1,7.5,54.7l32-8.2a21.2,21.2,0,0,1-6-14.7V23.3A23.4,23.4,0,0,1,59.9.2C71.4,1.7,80.1,12,80.1,24.1v7.7a21.2,21.2,0,0,1-6,14.7l31.8,8.1a12,12,0,0,1,7.7,11.2v84.6A6.5,6.5,0,0,1,107.1,156.9ZM56.8,5.9a17.7,17.7,0,0,0-11.5,4.3,17.3,17.3,0,0,0-5.9,13.1v8.5a15.1,15.1,0,0,0,7.7,13.3,2.9,2.9,0,0,1-.6,5.3l-37,9.8a6.1,6.1,0,0,0-3.6,5.6v84.6a.6.6,0,0,0,.6.6H107.1a.6.6,0,0,0,.6-.6V65.8a6.2,6.2,0,0,0-3.8-5.7L67.1,50.4a2.9,2.9,0,0,1-2-2.5,2.8,2.8,0,0,1,1.4-2.8,15.1,15.1,0,0,0,7.7-13.3V24.1c0-9.2-6.4-16.9-15-18.1Z"/>
							<path class="cls-1" d="M61.2,140H52.4A26.5,26.5,0,0,1,26,113.5V90A26.4,26.4,0,0,1,52.4,63.6h8.8A26.4,26.4,0,0,1,87.6,90v23.5A26.5,26.5,0,0,1,61.2,140ZM52.4,66.6A23.5,23.5,0,0,0,28.9,90v23.5A23.5,23.5,0,0,0,52.4,137h8.8a23.5,23.5,0,0,0,23.5-23.5V90A23.5,23.5,0,0,0,61.2,66.6Z"/>
							<path class="cls-1" d="M61.2,140H52.4A26.5,26.5,0,0,1,26,113.5V90A26.4,26.4,0,0,1,52.4,63.6h8.8A26.4,26.4,0,0,1,87.6,90v23.5A26.5,26.5,0,0,1,61.2,140ZM52.4,66.6A23.5,23.5,0,0,0,28.9,90v23.5A23.5,23.5,0,0,0,52.4,137h8.8a23.5,23.5,0,0,0,23.5-23.5V90A23.5,23.5,0,0,0,61.2,66.6Z"/>
							<path class="cls-1" d="M61.2,131.6H52.4a18.1,18.1,0,0,1-18.1-18.1V90A18.2,18.2,0,0,1,52.4,71.9h8.8A18.2,18.2,0,0,1,79.3,90v23.5A18.1,18.1,0,0,1,61.2,131.6ZM52.4,74.9A15.2,15.2,0,0,0,37.2,90v23.5a15.2,15.2,0,0,0,15.2,15.2h8.8a15.2,15.2,0,0,0,15.2-15.2V90A15.2,15.2,0,0,0,61.2,74.9Z"/>
							<path class="cls-1" d="M61.2,123.3H52.4a9.8,9.8,0,0,1-9.8-9.8V90a9.7,9.7,0,0,1,9.8-9.7h8.8A9.7,9.7,0,0,1,71,90v23.5A9.8,9.8,0,0,1,61.2,123.3ZM52.4,83.2A6.8,6.8,0,0,0,45.6,90v23.5a6.8,6.8,0,0,0,6.8,6.9h8.8a6.8,6.8,0,0,0,6.8-6.9V90a6.8,6.8,0,0,0-6.8-6.8Z"/>
							<path class="cls-1" d="M56.8,115a5.9,5.9,0,0,1-4.2-1.7,6.1,6.1,0,0,1-1.7-4.2V94.4a5.9,5.9,0,0,1,1.8-4.1,6.3,6.3,0,0,1,4.1-1.7,5.8,5.8,0,0,1,5.9,5.9v14.6A6,6,0,0,1,56.8,115Zm-2.9-5.9a2.7,2.7,0,0,0,2.9,2.9,2.9,2.9,0,0,0,2.9-2.9V94.5a2.9,2.9,0,0,0-2.9-3h0a2.9,2.9,0,0,0-2.1.9,3,3,0,0,0-.8,2Z"/>
						</g>
						</g>
					</g>
					</svg>
					<p class="absolute shadow-text ma0 f4 f3-l ttu">identity</p>
				</a>
				
			</div>
			<div class="pseudo-border absolute absolute-bottom"></div>
           
		</div>
		
		
        
        <div class=" alt-link-wrapper flex flex-column justify-start items-center relative contact-tile">
			<div class="h-100 w-100 flex justify-center items-center relative">
				<a href="/contact" class="inline-flex link  h-100 justify-center tom-nav-link" >
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 161.1 126.7">
						<g id="Layer_2" data-name="Layer 2">
							<g id="Layer_1-2" data-name="Layer 1">
							<g>
								<path class="cls-1" d="M131.9,126.7H29.2A29.2,29.2,0,0,1,0,97.5V29.2A29.2,29.2,0,0,1,29.2,0H131.9a29.2,29.2,0,0,1,29.2,29.2V97.5A29.2,29.2,0,0,1,131.9,126.7ZM29.2,6A23.2,23.2,0,0,0,6,29.2V97.5a23.2,23.2,0,0,0,23.2,23.2H131.9a23.2,23.2,0,0,0,23.2-23.2V29.2A23.2,23.2,0,0,0,131.9,6Z"/>
								<g>
								<path class="cls-1" d="M33.1,13.7H38v9.1h7.6V13.7h4.9V36.9H45.6V27.3H38v9.6H33.1Z"/>
								<path class="cls-1" d="M55.1,13.7H70v4.5H60v4.7h9.1v4.3H60v5.2H70v4.5H55.1Z"/>
								<path class="cls-1" d="M73.6,13.7h4.9V32.4h9.8v4.5H73.6Z"/>
								<path class="cls-1" d="M91.4,13.7h4.9V32.4h9.8v4.5H91.4Z"/>
								<path class="cls-1" d="M117.5,13.2c6.1,0,10.2,4.8,10.2,12.1s-4.1,12.1-10.2,12.1-10.3-4.8-10.3-12.1S111.3,13.2,117.5,13.2Zm0,4.5c-3.3,0-5.4,2.9-5.4,7.6s2.1,7.6,5.4,7.6,5.3-2.8,5.3-7.6S120.8,17.7,117.5,17.7Z"/>
								</g>
								<rect class="cls-1" x="3" y="44.7" width="155.1" height="5.99"/>
								<rect class="cls-1" x="3" y="93.5" width="155.1" height="5.99"/>
							</g>
							</g>
						</g>
					</svg>
					<p class="absolute shadow-text ma0 f4 f3-l ttu">contact</p>
				</a>
				
			</div>
		
			<div class="pseudo-border absolute absolute-bottom"></div>
            <div class="pseudo-border vertical absolute absolute-top absolute-right"></div>
		</div>
		
			<div class="pseudo-border absolute absolute-bottom"></div>
			<div class="pseudo-border vertical absolute absolute-top absolute-right"></div>
			<div class="pseudo-border vertical absolute absolute-top absolute-left"></div>
		</div>